...
def _dedup_trades_keep_first(trades: list[Trade]) -> list[Trade]:
    ...
    for t in sorted(trades, key=lambda x: x.timestamp):
        key = (
            ...
            t.side,
            t.contracts,
            round(float(t.entry), 6),
            round(float(t.exit), 6),
        )
        if key in seen:
            continue
        seen.add(key)
...
def run_walk_forward(
    ...
    while i + test_days <= len(days):
        ...
        all_oos_trades_raw.extend(oos_trades)
    ...
    raw_count = len(all_oos_trades_raw)
    all_oos_trades = _dedup_trades_keep_first(all_oos_trades_raw)
    dedup_count = len(all_oos_trades)
    ...
    return {
        ...
        "kpis": agg.get("kpis", {}),
        "apex": agg.get("apex", {}),
        "trade_count": agg.get("trade_count", dedup_count),
        "windows": len(window_reports),
        "raw_oos_trade_count": raw_count,
        "dedup_oos_trade_count": dedup_count,
        "duplicate_trades_removed": dup_removed,
...
def main() -> None:
    ...
    payload = {
        "strategy": "v2_3d_profile_edge_orderflow_walk_forward_step10_dedup_sensitivity",
        "strategy_config": asdict(BEST_CFG),
        "source_csv": str(csv_path),
        "dedup_method": "exact trade-key dedup across overlapping OOS windows: (timestamp, side, contracts, entry, exit)",
        "results": results,
        "ranked": ranked,
    ...
    with open(csv_out, "w", encoding="utf-8", newline="") as f:
        w = csv.writer(f)
        w.writerow([
            "train_days", "test_days", "step_days", "windows", "raw_oos_trades", "dedup_oos_trades", "duplicates_removed",
            "total_pnl", "win_rate", "profit_factor", "max_drawdown", "apex_passed"
        ])
        for r in ranked:
            s = r["schema"]
            k = r.get("kpis", {})
            a = r.get("apex", {})
            w.writerow([
                s["train_days"], s["test_days"], s["step_days"], r.get("windows"), r.get("raw_oos_trade_count"),
                r.get("dedup_oos_trade_count"), r.get("duplicate_trades_removed"),
    ...
    for r in ranked:
        ...
        k = r.get("kpis", {})
        rows.append(
            f"<tr><td>{s['train_days']}/{s['test_days']}/{s['step_days']}</td><td>{r.get('windows')}</td>"
            f"<td>{r.get('raw_oos_trade_count')}</td><td>{r.get('dedup_oos_trade_count')}</td><td>{r.get('duplicate_trades_removed')}</td>"
    ...
    html = (
        "<html><head><meta charset='utf-8'><title>v2.3d Walk-Forward Step10 Dedup</title>"
        ...
        "</head><body>"
        "<h1>v2.3d Walk-Forward Sensitivity (step=10, dedupliziert)</h1>"
        f"<p>Best Schema: {payload['best_schema']}</p>"
        "<table><tr><th>Schema train/test/step</th><th>Windows</th><th>Raw OOS</th><th>Dedup OOS</th><th>Removed</th><th>Total PnL</th><th>Winrate</th><th>PF</th><th>Max DD</th></tr>"
        + "".join(rows)
        + "</table></body></html>"
    )
    ...
    print("raw_oos:", best.get("raw_oos_trade_count"))
    print("dedup_oos:", best.get("dedup_oos_trade_count"))
    print("removed:", best.get("duplicate_trades_removed"))
