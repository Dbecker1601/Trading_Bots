...
def find_project_env(start: Path | None = None) -> Path | None:
    """Find the nearest .env from the current or module directory."""
    current = (start or Path.cwd()).resolve()
    module_dir = Path(__file__).resolve().parent
    candidates = [current, *current.parents, module_dir, *module_dir.parents]
...
def load_env_file(
    ...
    """Load dotenv-style KEY=VALUE pairs and return only key names."""
